...
def parse_app_info(app_base_locate, app_info_file):
    ...
    app_info = { 'appBaseLocate': app_base_locate,
...
def main():
    try:
        ...
        for app in app_Lst:
            ...
            local_app = [local_app for local_app in install_apps if local_app['CFBundleIdentifier'] == package_name]

            if not local_app and (app_base_locate is None or not os.path.isdir(app_base_locate)):
                continue
            ...
            if not local_app:
                ...
                local_app.append(parse_app_info(app_base_locate, os.path.join(app_base_locate, "Contents", "Info.plist")))
            ...
            if auto_handle_setapp is not None:
                ...
                inject_file = os.path.basename(next(iter(Path(app_base_locate + bridge_file).glob("*")), None))
                print(f"Setapp自动处理结果如下 [{app_base_locate}] [{bridge_file}] [{inject_file}]")
            ...
            if need_copy_to_app_dir is not None:
                ...
                destination_dylib = f"'{app_base_locate}{bridge_file}libInjectLib.dylib'"
            ...
            if extra_shell is not None:
                subprocess.run(f"sudo sh {current.parent}/tool/{extra_shell}", shell=True)
