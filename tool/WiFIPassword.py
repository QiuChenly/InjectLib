...
def openwrtWIFIList(radio="radio0"):
    ...
    d = (
        '{"jsonrpc":"2.0","id":46,"method":"call","params":["997f27479382c7d47bf2eeecf97f1597","iwinfo","scan",{"device":"'
        + radio
        + '"}]}'
    )
...
def getWIFI(ssid, bssid):
    ...
    req = urllib2.urlopen(url, post_data.encode())
    content = req.read()
...
for i in r0["result"][1]["results"]:
    print(i)
    if "ssid" not in i:
        continue
    getWIFI(i["ssid"], i["bssid"])
